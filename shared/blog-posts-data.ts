/**
 * shared/blog-posts-data.ts
 *
 * Single source of truth for published blog post content.
 * No browser-only imports (no asset paths, no image URLs) so this file is
 * safe to import from both the server (server/seo.ts) and the client
 * (client/src/data/blog-posts.ts).
 *
 * When adding a new article:
 *   1. Add a new entry to sharedBlogPosts below.
 *   2. Server-side SEO (title, description, Article JSON-LD, body HTML) is
 *      derived automatically — no changes to server/seo.ts required.
 */
...
export const sharedBlogPosts: SharedBlogPost[] = [
  {
    id: '1',
