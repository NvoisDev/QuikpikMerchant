...
- **Enhanced Stripe Connect with Separate Delivery Fee Routing (Aug 2025)**: Implemented comprehensive Stripe Connect destination charges with automatic payment splitting. Option A (Connect enabled): Customer payment automatically splits into wholesaler product earnings (via transfer_data) and platform fees (via application_fee_amount). Separate transfer logic routes delivery fees + transaction fees directly to platform account for immediate Parcel2Go prepaid funding. Option B (No Connect): Standard payment intent with manual payout handling. Corrected transaction fee calculation to 5.5% of (products + delivery) + £0.50 for comprehensive fee coverage. System now supports both automatic marketplace payments and fallback manual processing.
