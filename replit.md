...
- **Critical Stripe Connect Destination Charges Fix (Aug 2025)**: Corrected payment intent creation to follow official Stripe Connect documentation. Fixed issue where wholesalers received full customer payment instead of net earnings by properly implementing destination charges pattern. Now using only application_fee_amount (platform keeps all fees + delivery costs) with automatic transfer calculation for wholesaler product earnings. Removed conflicting transfer_data.amount parameter and enforced Stripe Connect requirement for all payments.
