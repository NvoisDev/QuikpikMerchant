...
- **Revolutionary Products-Only Stripe + Separate Delivery Collection (Aug 2025)**: Completely restructured payment system to exclude delivery costs from Stripe Connect formula and implement separate platform-direct delivery collection. Stripe now processes ONLY product payments (subtotal + 5.5% transaction fee on products only + £0.50). Platform collects delivery fees through separate payment intents directly to platform account for centralized Parcel2Go funding. This eliminates all Stripe Connect delivery routing complexity while maintaining proper marketplace fee structure. Wholesalers receive 96.7% of product earnings only. Platform manages all delivery costs independently.
- **CRITICAL ORDER TAGGING FIX (Aug 2025)**: Resolved critical fulfillmentType detection bug where delivery orders were incorrectly tagged as pickup orders. Issue was in order-processor.ts line 183 - system was checking for shippingCost in payment metadata to detect delivery, but separate delivery payment system means no shipping cost in Stripe metadata. Fixed to use shippingInfo.option directly from payment intent metadata, ensuring accurate pickup vs delivery classification in order history. Added visual shipping method badges to cart summary (green pickup, blue delivery) for immediate user feedback.
- **REVOLUTIONARY V2 STRIPE CONNECT MIGRATION (Aug 2025)**: Completed comprehensive migration from old Stripe Connect architecture to revolutionary V2 "Separate Charges and Transfers" system. Platform now receives ALL funds first (products + delivery fees), automatically distributes wholesaler share after deducting different percentage-based platform fees from both customer and wholesaler, while maintaining centralized delivery payment control through Parcel2Go prepaid accounts. MILESTONE ACHIEVED: Systematically removed entire legacy Stripe Connect flow from codebase, cleaned up all old payment processing endpoints and unreachable code sections. Platform now operates exclusively on V2 system with 100% fund control, simplified processing, and complete audit trail. V2 test interface available at /stripe-v2-test for system validation. Legacy endpoints return HTTP 410 Gone with migration guidance.
...
- **Payment Processing**: Stripe Connect V2 "Separate Charges and Transfers" architecture (platform-first fund collection with automatic wholesaler distributions and complete payment audit trail).
