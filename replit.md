...
- **Stripe Connect Application Fee Bug Discovery (Aug 2025)**: Identified critical bug where Stripe Connect application_fee_amount only received wholesaler platform fee (£11.30) instead of total platform fees (£268.23 for SF-169). This caused wholesalers to receive £588.02 instead of correct £331.09. Bug analysis shows code calculations are mathematically correct but only partial fee amount reaches Stripe API. Added enhanced debugging and recalculation verification to fix application fee transmission issue.
