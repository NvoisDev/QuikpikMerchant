...
- **Delivery Cost Capture Fix (Aug 2025)**: Resolved critical issue where new delivery orders weren't capturing delivery costs in database (SF-175, SF-176 required manual corrections). Implemented robust multi-tier fallback system in order processor: 1) Use paymentIntent.metadata.shippingCost if > 0, 2) Use shippingInfo.service.price if available, 3) Default £90.00 for delivery orders without specific pricing. Fixed fulfillment type detection and total calculation to consistently include shipping costs. Enhanced logging to track payment metadata and shipping cost processing for future debugging.
