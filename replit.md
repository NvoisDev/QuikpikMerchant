...
- **Critical Wholesaler Transfer Amount Fix (Aug 2025)**: Resolved major payment calculation error where wholesalers received total customer payment minus platform fee instead of subtotal minus platform fee. Previously order SF-167 transferred £1,909.45 to wholesaler (should be £1,634.23). Fixed calculation ensures wholesaler receives only subtotal minus 3.3% platform fee, while platform correctly retains transaction fees, delivery costs, and platform fees. This maintains proper marketplace fee structure where wholesalers earn 96.7% of product revenue only.
- **Stripe Connect Application Fee Bug Discovery & Fix (Aug 2025)**: Identified and resolved critical bug where Stripe Connect application_fee_amount only received wholesaler platform fee (£11.30) instead of total platform fees (£268.23 for SF-169). Root cause: Customer portal orders were using wrong endpoint (/api/marketplace/create-payment-intent) that only applied platform fee, not transaction fees and delivery costs. Fixed by updating marketplace endpoint to calculate totalPlatformFees = platformFee + customerTransactionFee + shippingCost and pass finalApplicationFeeAmount to Stripe. Added comprehensive debugging to verify correct fee transmission. This ensures wholesalers receive proper earnings (96.7% of product subtotal) while platform retains all fees.
