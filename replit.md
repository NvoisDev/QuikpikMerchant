...
- **Authentication**: Dual system (Google OAuth for wholesalers, SMS for customer portal). Role-based access control enforces data isolation. **Critical Fix Completed**: Customer SMS authentication now properly queries `wholesaler_customer_relationships` table for multi-wholesaler platform compatibility.
