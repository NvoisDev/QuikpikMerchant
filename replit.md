...
- **Delivery Cost Capture Fix (Aug 2025)**: Resolved critical issue where delivery orders appeared as pickup during payment processing (SF-175, SF-176, SF-177 required manual corrections). Root cause was frontend race condition where payment intents were created before shipping service selection was complete. Fixed by: 1) Enhanced payment intent creation validation to ensure complete shipping data, 2) Added selectedShippingService to useEffect dependencies, 3) Implemented robust backend fallback system for order processor, 4) Enhanced logging for debugging. Payment intents now wait for complete shipping data before creation, preventing delivery orders from being processed as pickup.
