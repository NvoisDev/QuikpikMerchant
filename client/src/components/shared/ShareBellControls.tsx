...
interface ShareBellControlsProps {
  variant?: "light" | "dark";
}

export default function ShareBellControls({ variant = "light" }: ShareBellControlsProps) {
  const { user } = useAuth();
  ...
  const buttonClass = isDark
    ? "relative text-slate-400 hover:text-white hover:bg-slate-800 h-9 w-9"
    : "relative hover:bg-slate-100 text-slate-500 hover:text-slate-700";
