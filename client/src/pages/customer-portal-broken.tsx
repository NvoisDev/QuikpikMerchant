...
const formatCurrencyAmount = (amount: number, currency = 'GBP'): string => {
...
// SEO Meta component
const SEOMeta = ({ product, wholesaler }: { product?: any; wholesaler?: any }) => {
  useEffect(() => {
    if (product && wholesaler) {
      document.title = `${product.name} - ${wholesaler.businessName} | Quikpik`;
      
      // Update meta description
      const metaDescription = document.querySelector('meta[name="description"]');
      if (metaDescription) {
        metaDescription.setAttribute('content', 
          `Shop ${product.name} from ${wholesaler.businessName}. ${product.description || 'Quality wholesale products'} - Starting from ${formatCurrencyAmount(parseFloat(product.price), wholesaler.defaultCurrency)}`
        );
      }
    } else {
      document.title = 'Wholesale Marketplace | Quikpik';
    }
  }, [product, wholesaler]);
...
export default function CustomerPortal() {
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                Pay {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{getTotalAmount().toFixed(2)}
              </>
            )}
          </Button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEOMeta product={featuredProduct} wholesaler={wholesaler} />
      ...
      {/* Customer Portal Header - Mobile Optimized */}
      <div className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            <div className="min-w-0 flex-1">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">
                {featuredProduct?.wholesaler?.businessName || allProducts[0]?.wholesaler?.businessName || 'Our Store'}
              </h1>
              <p className="text-sm text-gray-600 hidden sm:block">Quality products at wholesale prices</p>
            </div>
            ...
            <div className="flex items-center justify-between sm:justify-end space-x-3 sm:space-x-4">
              <div className="flex items-center space-x-2 text-gray-600 min-w-0">
                ...
                <span className="text-xs sm:text-sm truncate">
                  {cartStats.totalItems} {cartStats.totalItems === 1 ? 'item' : 'items'} • {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{cartStats.totalValue.toFixed(2)}
                </span>
              </div>
              {cart.length > 0 && (
                <Button 
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-xl">{getCurrencySymbol(wholesaler?.defaultCurrency)}{parseFloat(featuredProduct.price).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
              ...
              <div className={`${viewMode === "grid" 
                ...
                {(() => {
                  ...
                  return productsToShow.map((product) =>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="text-xs text-gray-600 space-y-1">
                            <div className="flex justify-between">
                              <span>Price:</span>
                              <span className="font-medium">{getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{parseFloat(product.price).toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>MOQ:</span>
                    ...
                    // List View - Add list view component here if needed
                    <Card key={product.id} className="border hover:shadow-lg transition-shadow">
                      <CardContent className="p-4">
                        <div className="text-center">List view coming soon</div>
                      </CardContent>
                    </Card>
                  )
                  );
                    ...
                    <div key={item.product.id} className="flex items-center justify-between p-2 border rounded">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-600">
                          {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{parseFloat(item.product.price).toFixed(2)} × {item.quantity} units
                        </div>
                      </div>
                      ...
                      <div className="w-20 text-right font-medium">
                        {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{(parseFloat(item.product.price) * item.quantity).toFixed(2)}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="border-b pb-3">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  <div>Price: {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{parseFloat(selectedProduct.price).toFixed(2)} per unit</div>
              ...
              <div className="bg-gray-50 p-3 rounded">
                <div className="flex justify-between items-center font-semibold">
                  <span>Total:</span>
                  <span>{getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{(parseFloat(selectedProduct.price) * editQuantity).toFixed(2)}</span>
                </div>
              </div>
              ...
              <div className="space-y-2 text-sm">
                {cart.map((item, index) => (
                  <div key={index} className="flex justify-between">
                    ...
                    <span>{getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{(parseFloat(item.product.price) * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
                <Separator />
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-3 rounded">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Listed at {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{negotiationProduct.price}
                    </p>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-2.5 text-gray-500">{getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}</span>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                {negotiationData.offeredPrice && (
                  <p className="text-xs text-gray-500">
                    Total: {getCurrencySymbol(wholesaler?.defaultCurrency || "GBP")}{(parseFloat(negotiationData.offeredPrice) * (typeof negotiationData.quantity === 'string' ? parseInt(negotiationData.quantity) || 1 : negotiationData.quantity)).toFixed(2)}
                  </p>
                )}
              </div>
