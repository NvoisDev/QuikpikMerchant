...
function MarginOverview() {
  ...
  const { data: drillData, isLoading: drillLoading } = useQuery<ProductOrderLine[]>({
    queryKey: ["/api/analytics/margin-product-orders", selectedMarginProduct?.productId, dateRange.from.toISOString(), dateRange.to.toISOString()],
    queryFn: async () => {
      const params = new URLSearchParams({
        productId: String(selectedMarginProduct!.productId),
        fromDate: dateRange.from.toISOString(),
        toDate: dateRange.to.toISOString(),
      });
      const res = await fetch(`/api/analytics/margin-product-orders?${params}`, { credentials: "include" });
      if (!res.ok) throw new Error("Failed to fetch product orders");
      return res.json();
    },
    enabled: selectedMarginProduct !== null,
    staleTime: 2 * 60 * 1000,
    retry: false,
  });
          ...
          <SheetHeader className="px-6 py-4 border-b border-slate-200 shrink-0">
            ...
            {selectedMarginProduct && (
              <div className="flex flex-wrap gap-4 mt-1 text-xs text-slate-500">
                ...
                {selectedMarginProduct.hasCost && (
                  <>
                    <span>Cost: <span className="font-medium text-slate-700">{fmt(selectedMarginProduct.cost)}</span></span>
                    <span>Margin: <span className={`font-semibold ${(selectedMarginProduct.marginPercent ?? 0) < 0 ? "text-red-600" : (selectedMarginProduct.marginPercent ?? 0) < 15 ? "text-amber-600" : "text-emerald-600"}`}>{fmt(selectedMarginProduct.margin)} ({pct(selectedMarginProduct.marginPercent ?? 0)})</span></span>
                  </>
                )}
              </div>
            )}
              ...
              <div className="mt-5 pt-4 border-t border-slate-200/70">
                ...
                <div className="grid grid-cols-2 gap-2 lg:hidden">
                  ...
                  <Link href="/customers?add=true">
                    <button className="w-full flex items-center justify-center gap-2 bg-blue-50 hover:bg-blue-100 active:bg-blue-200 text-blue-700 font-medium rounded-xl h-12 text-sm border border-blue-200 transition-colors">
                      <UserPlus className="h-4 w-4 shrink-0" />
                      Add Customer
                    </button>
                  </Link>
                  <Link href="/orders">
                    <button className="w-full flex items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 text-gray-700 font-medium rounded-xl h-11 text-sm border border-gray-200 transition-colors">
                ...
                <div className="hidden lg:flex items-center gap-2 flex-wrap">
                  ...
                  <Link href="/customers?add=true">
                    <button className="flex items-center gap-2 bg-blue-50 hover:bg-blue-100 active:bg-blue-200 text-blue-700 font-medium rounded-lg px-4 h-9 text-sm border border-blue-200 transition-colors">
                      <UserPlus className="h-4 w-4 shrink-0" />
                      Add Customer
                    </button>
                  </Link>
                  <Link href="/orders">
                    <button className="flex items-center gap-2 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 text-gray-600 font-medium rounded-lg px-4 h-9 text-sm border border-gray-200 transition-colors">
                      <ShoppingCart className="h-4 w-4 shrink-0" />
                      Orders
                    </button>
                  </Link>
