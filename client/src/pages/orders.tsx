...
// Add missing TimelineItem component
const TimelineItem = ({ icon: Icon, title, time, completed, current }: {
  icon: any;
  title: string;
  time?: string;
  completed: boolean;
  ...
  <div className={`flex items-start gap-3 ${current ? 'bg-blue-50 p-2 rounded-lg' : ''}`}>
    <div className={`p-1.5 rounded-full ${completed ? 'bg-green-100 text-green-600' : current ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
      <Icon className="h-3 w-3" />
    </div>
    <div className="flex-1">
      <div className={`text-sm font-medium ${completed ? 'text-green-900' : current ? 'text-blue-900' : 'text-gray-500'}`}>
        {title}
      </div>
      {time && (
...
export default function Orders() {
  ...
  // TimelineItem component for order timeline display
  const TimelineItem = ({ 
    icon, 
    title, 
    time, 
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <TimelineItem
                        ...
                        time={new Date(selectedOrder.createdAt).toLocaleString('en-GB', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                          hour: '2-digit',
                      ...
                      <TimelineItem
                        ...
                        completed={false}
                        current={selectedOrder.status === 'confirmed' || selectedOrder.status === 'pending'}
                      />
                      <TimelineItem
                        icon={selectedOrder.fulfillmentType === 'pickup' ? Hand : Truck}
                        title={selectedOrder.fulfillmentType === 'pickup' ? 'Package for collection' : 'Package for delivery'}
                        completed={false}
                      />
                      <TimelineItem
                        icon={CheckCircle}
                        title="Mark as fulfilled when ready"
                        completed={selectedOrder.status === 'fulfilled'}
                      />
                    </div>
                  </div>
