...
export async function sendPasswordResetEmail(email: string, token: string, firstName?: string): Promise<void> {
  ...
  const resetBody = `
    ${emailHeading('Reset Your Password', { size: '22px' })}
    ${firstName ? `<p style="font-size: 16px; margin: 0 0 8px 0;">Hi ${firstName},</p>` : ''}
    <p style="margin: 0 0 20px 0;">We received a request to reset your password for your Quikpik account. Click the button below to create a new password:</p>

    ${emailButton('Reset Password', resetUrl)}

    ${emailCard(`
      <p style="margin: 0; color: #6b7280; font-size: 14px;">This link will expire in 1 hour for security purposes. If you didn't request this password reset, you can safely ignore this email.</p>
    `)}
