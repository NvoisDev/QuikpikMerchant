...
export function wrapCustomerEmail(body: string, branding: EmailBranding, options?: { preheader?: string }): string {
  ...
  const logoBlock = buildLogoBlock(branding);
  const headerContent = logoBlock
    ? `${logoBlock}<p style="margin:8px 0 0;${F};font-size:18px;font-weight:700;color:#1f2937">${branding.businessName}</p>`
    : `<p style="margin:0;${F};font-size:20px;font-weight:700;color:#1f2937">${branding.businessName}</p>`;
  const raw = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#f4f5f7">${preheader ? `<div style="display:none;font-size:1px;color:#f4f5f7;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden">${preheader}</div>` : ''}<table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f5f7"><tr><td align="center" style="padding:24px 16px"><table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:12px;overflow:hidden"><tr><td style="padding:24px 40px 20px;text-align:center;border-bottom:1px solid #f0f0f0">${headerContent}</td></tr><tr><td style="padding:24px 40px;${F};font-size:15px;line-height:1.6;color:#374151">${body}</td></tr><tr><td style="padding:20px 40px 24px;background:#fafafa;border-top:1px solid #f0f0f0;text-align:center;${F}"><p style="margin:0 0 6px;font-size:13px;color:#9ca3af">Questions? Contact <strong style="color:#6b7280">${branding.businessName}</strong> directly.</p><p style="margin:12px 0 0;font-size:11px;color:#d1d5db">Powered by <a href="https://quikpik.co" style="color:#d1d5db;text-decoration:none;font-weight:600">Quikpik Merchant</a></p></td></tr></table></td></tr></table></body></html>`;
