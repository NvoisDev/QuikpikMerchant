...
<body>
  ...
  app.post('/api/team-members/invite', requireAuth, async (req: any, res) => {
    try {
      ...
      let limit = 0;
      if (subscriptionStatus.tier === 'standard') limit = 1;
      if (subscriptionStatus.tier === 'premium') limit = 5;
      
      if (currentCount >= limit) {
        return res.status(403).json({ 
          message: `You've reached your team member limit of ${limit}. Upgrade your plan to add more members.` 
        });
      }
  ...
  app.put('/api/team-members/:id/status', requireAuth, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const { id } = req.params;
      const { status } = req.body;
      
      const teamMember = await storage.updateTeamMemberStatus(parseInt(id), status);
      res.json(teamMember);
    } catch (error) {
