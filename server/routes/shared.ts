  ...
  try {
    ...
    if (wholesaler.email) {
      try {
        ...
        const wholesalerSubjectLabel = isDeposit ? 'Deposit Received' : isBalancePayment ? 'Balance Payment Received' : order.isQuote ? 'Payment Received' : 'New Order Received';
        const wholesalerBodyLabel = isDeposit
          ...
          : isBalancePayment
            ...
            : order.isQuote
              ? `Payment received from <strong>${customerDisplayName}</strong> for quote ${order.orderNumber || `#${order.id}`}. Invoice attached as PDF.`
