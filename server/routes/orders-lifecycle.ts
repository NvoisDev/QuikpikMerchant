...
export function registerOrderLifecycleRoutes(app: Express): void {
  ...
  // PUT /api/orders/:orderId/switch-to-collection
  app.put('/api/orders/:orderId/switch-to-collection', requireAuth, requireNotViewer, requireMemberPermission('orders'), async (req: any, res) => {
    try {
      ...
      if (order.wholesalerId !== wholesalerId) return res.status(403).json({ error: 'Not authorized' });
      if (order.status === 'cancelled') return res.status(400).json({ error: 'Cannot modify a cancelled order' });
      if (order.fulfillmentType === 'pickup') return res.status(400).json({ error: 'Order is already set to collection' });

      const updated = await storage.switchOrderToCollection(orderId);
      return res.json({ success: true, order: updated });
    } catch (error) {
      console.error('❌ switch-to-collection error:', error);
      return res.status(500).json({ error: 'Failed to switch order to collection' });
    }
  });
