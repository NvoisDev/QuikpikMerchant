...
import { customerFeeLabel } from "../shared/utils/fees";
...
export async function sendOrderConfirmationEmail(orderData: {
  ...
  const body = `<p style="font-size:16px;margin:0 0 8px">Dear ${orderData.customerName},</p><p style="margin:0 0 20px">Thank you for your order. We're pleased to confirm your order has been received and is being processed.</p>${emailCard(`${emailHeading('Order Details')}<p style="margin:0 0 6px"><strong>Order Number:</strong> ${orderData.orderNumber}</p>${orderData.shippingAddress ? `<p style="margin:0 0 6px"><strong>Shipping to:</strong> ${orderData.shippingAddress}</p>` : ''}${orderData.estimatedDelivery ? `<p style="margin:0 0 6px"><strong>Estimated Delivery:</strong> ${orderData.estimatedDelivery}</p>` : ''}`)}${emailTable(['Product', 'Qty', 'Unit Price', 'Total'], itemRows)}${emailCard(`<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse"><tr><td style="padding:6px 0"><strong>Subtotal:</strong></td><td style="padding:6px 0;text-align:right;font-weight:600">£${orderData.subtotal.toFixed(2)}</td></tr><tr><td style="padding:6px 0;color:#6b7280;font-size:14px">Service Fee (${customerFeeLabel()}):</td><td style="padding:6px 0;text-align:right;color:#6b7280;font-size:14px">£${orderData.transactionFee.toFixed(2)}</td></tr><tr style="border-top:2px solid #e5e7eb"><td style="padding:12px 0 4px;font-size:17px;font-weight:700">Total Paid:</td><td style="padding:12px 0 4px;text-align:right;font-size:17px;font-weight:700;color:#10b981">£${orderData.totalPaid.toFixed(2)}</td></tr></table>`)}${paymentTermsHtml}${emailCard(`<p style="margin:0;color:#0f766e;font-size:14px"><strong>Stripe Receipt:</strong> You'll receive a separate payment receipt from Stripe at this email address.</p>`, { borderColor: '#a7f3d0', bgColor: '#ecfdf5' })}`;
